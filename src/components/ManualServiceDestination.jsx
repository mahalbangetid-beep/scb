...
export default function ManualServiceDestination({ panelId }) {
    ...
    const handleSave = async () => {
        ...
        setError(null)
        try {
            await api.post(`/provider-config/manual-destination`, { panelId, ...form })
    ...
    return (
        <div>
            <div style={{ marginBottom: 'var(--spacing-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                {isSaved && (
                    <span style={{ background: 'var(--color-success)', color: '#fff', padding: '4px 12px', borderRadius: 'var(--radius-sm)', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <button className="btn btn-primary" onClick={handleSave} disabled={saving} style={{ minWidth: 160 }}>
                        ...
                        {isSaved ? ' Update Destination' : ' Save Destination'}
                    </button>
                </>
            )}
        </div>
